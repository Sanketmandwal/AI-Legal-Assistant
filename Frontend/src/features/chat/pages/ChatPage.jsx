...
import { MessageSquare, Send, ArrowLeft, User, Paperclip, Loader2, FileText, Image, File, Eye } from 'lucide-react'
        ...
        <div className={`rounded-2xl px-4 py-2.5 text-sm ${isMe ? 'rounded-br-md bg-primary text-primary-foreground' : 'rounded-bl-md border border-border bg-card text-foreground shadow-sm shadow-slate-950/5'}`}>
          ...
          {msg.attachments?.length > 0 && (
            <div className="mt-2 space-y-1">{msg.attachments.map((a, i) => {
              const displayName = friendlyDocName(null, a.filename, i) || 'Attachment'
              const isImage = a.signedUrl && (a.signedUrl.match(/\.(jpg|jpeg|png|gif|webp)(\?|$)/i) || a.signedUrl.includes('/image/'))
              return (
                <button
                  key={i}
                  onClick={() => onAttachmentClick(msg.attachments, i)}
                  className={`flex items-center gap-1.5 text-xs ${isMe ? 'text-blue-100 hover:text-white' : 'text-blue-600 hover:text-blue-800'} transition-colors`}
                >
                  {isImage ? <Image className="h-3 w-3" /> : <Paperclip className="h-3 w-3" />}
...
export default function ChatPage() {
  ...
  const handleAttachmentClick = (attachments, index) => {
    const docs = attachments.map((a, i) => ({
      url: a.signedUrl || '#',
      filename: a.filename,
          ...
          {!roomId ? (
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto bg-muted/35 p-4">
                ...
                ) : messages.map((msg) => <MessageBubble key={msg._id} msg={msg} isMe={String(msg.senderId?._id) === String(user?._id)} onAttachmentClick={handleAttachmentClick} />)}
