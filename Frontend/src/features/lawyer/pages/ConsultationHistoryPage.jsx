    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <div className="space-y-3">
          {consultations.map((c) => (
            <Card key={c._id} className=" hover:border-primary/30 transition-all">
              <CardContent className="p-5">
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-2">
                      <div className="h-8 w-8 rounded-full bg-primary/10 flex items-center justify-center text-primary text-xs font-bold">{c.citizen?.name?.[0] || '?'}</div>
                  ...
                  {c.status === 'accepted' && c.chatRoom && (
                    <Button size="sm" asChild className=""><Link to={`/lawyer/chat/${c.chatRoom._id}`}><MessageSquare className="mr-1.5 h-3.5 w-3.5" /> Chat</Link></Button>
                  )}
                </div>
              </CardContent>
            </Card>
